...
class ST_TazerProj extends TazerProjectile;
...
{	
	...
	{
		if (Level.NetMode == NM_Client && !IsA('NN_TazerProjOwnerHidden'))
		{
...
simulated function NN_SuperExplosion(Pawn Pwner)	// aka, combo.
{
	...
	Ring = Spawn(Class'RingExplosion2',Pwner,'',Location, Tater);
	PlaySound(ExploSound,,20.0,,2000,0.6);
    if(bbP != none)
    {
        bbP.xxClientDemoFix(Ring, class'RingExplosion2', Location, Ring.Velocity, Ring.Acceleration, Tater);
    }
...
simulated function Explode(vector HitLocation, vector HitNormal)
{
	...
	if (Damage > 60)
	{
		...
		if (bbP != None)
			bbP.xxClientDemoFix(None, class'RingExplosion3',HitLocation+HitNormal*8,,, rotator(HitNormal));
	}
	else
	{
		Spawn(class'RingExplosion5',,, HitLocation+HitNormal*8,rotator(Velocity));
		if (bbP != None)
			bbP.xxClientDemoFix(None, class'RingExplosion5',HitLocation+HitNormal*8,,, rotator(Velocity));
	}
...
simulated function NN_Momentum( float DamageRadius, float Momentum, vector HitLocation )
{
	...
	if ( bbP == None || !bbP.bNewNet || Self.IsA('NN_TazerProjOwnerHidden') || RemoteRole == ROLE_Authority )
		return;
