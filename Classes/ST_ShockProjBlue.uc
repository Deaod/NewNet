class ST_ShockProjBlue extends BlueShockProj;
...
{	
	...
	if(Pawn(Owner) != None)
	{
		Spawn(Class'Blue_ComboRing',,'',Location, Pawn(Owner).ViewRotation);
	}
...
{	
    ...
    {
    	BRing = Spawn(Class'Blue_SuperComboRing',,'',Location, Pawn(Owner).ViewRotation);
	}
	PlayOwnedSound(ExploSound,,20.0,,2000,0.6);
...
simulated function NN_SuperExplosion(Pawn Pwner)	// aka, combo.
{
	...
	{
		BlueRing = Spawn(Class'Blue_ComboRing',Pawn(Owner),'',Location, Tater);
	}
	PlaySound(ExploSound,,20.0,,2000,0.6);
    if(bbP != none && Pawn(Owner) != None)
    {
    	bbP.xxClientDemoFix(BlueRing, class'Blue_ComboRing', Location, BlueRing.Velocity, BlueRing.Acceleration, Tater);		
    }
...
simulated function NN_SuperDuperExplosion(Pawn Pwner)	// aka, combo.
{
	...
	if(bbP != none && Pawn(Owner) != None)
    {
    	BRing = Spawn(Class'Blue_SuperComboRing',Pawn(Owner),'',Location, Tater);
	}
	PlaySound(ExploSound,,20.0,,2000,0.6);
    if(bbP != none && Pawn(Owner) != None)
    {
    	bbP.xxClientDemoFix(BRing, Class'Blue_SuperComboRing', Location, BRing.Velocity, BRing.Acceleration, Tater);
    }
...
simulated function Explode(vector HitLocation, vector HitNormal)
{
	...
	if (Damage > 60)
	{
		...
		if (bbP != None && Pawn(Owner) != None)
		{
			bbP.xxClientDemoFix(None, class'Blue_RingExplosion',HitLocation+HitNormal*8,,, rotator(HitNormal));
		}
	}
	else
	{
