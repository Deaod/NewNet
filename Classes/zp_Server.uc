...
state zp_Gun
{
	...
	simulated function Actor xxProcessTraceHit (Actor zzOther, Vector zzHitLocation, Vector zzHitNormal, Vector X, Vector Y, Vector Z)
	{
		...
		if ( (zzOther == None) && OnClient() )
		{
			if ( zzW.IsA('ST_SniperRifle') )
			{
				ST_SniperRifle(zzW).xx_(0,zzTime,False);
			}
			else
			{
		...
		if ( OnClient() )
		{
			...
			if ( OnClient() )
			{
				if ( zzW.IsA('ST_SniperRifle') )
				{
					...
					{
						ST_SniperRifle(zzW).xx_(zzHash,zzTime,True);
					}
					else
					{
						ST_SniperRifle(zzW).xx_(zzHash,zzTime,False);
					}
				}
				else
				{
