class NN_ShockProjRedOwnerHidden extends ST_ShockProjRed;
		...
		for (P = Level.PawnList; P != None; P = P.NextPawn)
			if (P != Instigator) {
				if (Dmg > 60)
					...
					CR = P.Spawn(class'Red_RingExplosion',P,, HitLocation+HitNormal*8,rotator(Velocity));
...
simulated function DoSuperExplosion()
{
	...
	if (RemoteRole < ROLE_Authority)
	{
		for (P = Level.PawnList; P != None; P = P.NextPawn)
		{
			if (P != Owner)
			{
				CR = P.Spawn(Class'Red_ComboRing',P,'',Location, Pawn(Owner).ViewRotation);
				CR.bOnlyOwnerSee = True;
			}
		}
	}
...
simulated function DoSuperDuperExplosion()
{
	...
	if (RemoteRole < ROLE_Authority) {
		for (P = Level.PawnList; P != None; P = P.NextPawn)
		{
			if (P != Owner)
			{
				CR = P.Spawn(Class'Red_SuperComboRing',P,'',Location, Pawn(Owner).ViewRotation);
				CR.bOnlyOwnerSee = True;
			}
		}
	}
