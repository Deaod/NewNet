class ST_sgShockProjGold extends ST_sgShockProj;

simulated function Tick(float DeltaTime)
{
	Texture = Texture'Go_a00';
	LightBrightness=255;
	LightHue=40;
	LightSaturation=50;
	LightRadius=6;
...
{	
	...
	{
		if (Level.NetMode == NM_Client && !IsA('NN_sgShockProjGoldOwnerHidden'))
		{
	...
	Spawn(Class'Gold_ComboRing',,'',Location, Instigator.ViewRotation);
...
{	
	local bbPlayer bbP;
    local Gold_SuperComboRing Ring;
	...
	Ring = Spawn(Class'Gold_SuperComboRing',,'',Location, Instigator.ViewRotation);
	PlayOwnedSound(ExploSound,,20.0,,2000,0.6);
...
simulated function NN_SuperExplosion(Pawn Pwner)	// aka, combo.
{
	...
	Ring = Spawn(Class'Gold_ComboRing',Pwner,'',Location, Tater);
	PlaySound(ExploSound,,20.0,,2000,0.6);
    if(bbP != none)
    {
        bbP.xxClientDemoFix(Ring, class'Gold_ComboRing', Location, Ring.Velocity, Ring.Acceleration, Tater);
    }
...
simulated function NN_SuperDuperExplosion(Pawn Pwner)	// aka, combo.
{
	...
	Ring = Spawn(Class'Gold_SuperComboRing',Pwner,'',Location, Tater);
	PlaySound(ExploSound,,20.0,,2000,0.6);
    if(bbP != none)
        bbP.xxClientDemoFix(Ring, Class'Gold_SuperComboRing', Location, Ring.Velocity, Ring.Acceleration, Tater);
...
simulated function Explode(vector HitLocation, vector HitNormal)
{
	...
	if (Damage > 60)
	{
		...
		if (bbP != None)
			bbP.xxClientDemoFix(None, class'UT_GoldRingExplosion2',HitLocation+HitNormal*8,,, rotator(HitNormal));
	}
	else
	{
		Spawn(class'UT_GoldRingExplosion2',,, HitLocation+HitNormal*8,rotator(Velocity));
		if (bbP != None)
			bbP.xxClientDemoFix(None, class'UT_GoldRingExplosion2',HitLocation+HitNormal*8,,, rotator(Velocity));
	}
...
simulated function NN_Momentum( float DamageRadius, float Momentum, vector HitLocation )
{
	...
	if ( bbP == None || !bbP.bNewNet || Self.IsA('NN_sgShockProjGoldOwnerHidden') || RemoteRole == ROLE_Authority )
		return;
