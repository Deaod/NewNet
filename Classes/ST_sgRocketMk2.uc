...
class ST_sgRocketMk2 extends RocketMk2;
...
auto state Flying
{
	...
	simulated function BlowUp(vector HitLocation)
	{
		...
		{
			if (Level.NetMode == NM_Client && !IsA('NN_sgrocketmk2OwnerHidden'))
	...
	simulated function NN_Momentum( float DamageRadius, float Momentum, vector HitLocation )
	{
		...
		if ( bbP == None || !bbP.bNewNet || Self.IsA('NN_sgrocketmk2OwnerHidden') || RemoteRole == ROLE_Authority )
			return;
