class ST_UTApeChunk extends UTChunk;

var ST_UTApeChunkInfo Chunkie;
var int ChunkIndex;
var float R1, R2, R3, R4, R5, R6, R7, R8, R9, R10, R11, R12, R13, R14, R15, R16;
...
simulated function ProcessTouch (Actor Other, vector HitLocation)
{
	...
	if ( (Chunk(Other) == None) && ((Physics == PHYS_Falling) || (Other != Instigator || bHitWall)) && (Other != Owner/*  && Other.Owner != Owner */ || bHitWall) )
	{
		...
		If ( speed > 200 )
		{
			...
			if (bbP != None && bbP.bNewNet && Level.NetMode == NM_Client && !IsA('NN_UTApeChunkOwnerHidden'))
			{
				NN_HitOther = Other;
...
simulated function NN_Momentum( vector Momentum, vector HitLocation )
{
	...
	if ( bbP == None || !bbP.bNewNet || Self.IsA('NN_UTApeChunkOwnerHidden') || RemoteRole == ROLE_Authority )
		return;
