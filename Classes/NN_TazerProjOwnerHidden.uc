class NN_TazerProjOwnerHidden extends ST_TazerProj;
...
simulated function DoExplode(int Dmg, vector HitLocation,vector HitNormal)
{
	...
	if (RemoteRole < ROLE_Authority) {
		...
		ForEach AllActors(class'PlayerPawn', P)
			if (P != Instigator) {
				if (Dmg > 60)
					CR = P.Spawn(class'RingExplosion3',P,, HitLocation+HitNormal*8,rotator(HitNormal));
				else
					CR = P.Spawn(class'RingExplosion5',P,, HitLocation+HitNormal*8,rotator(Velocity));
...
simulated function DoSuperExplosion()
{
	...
	if (RemoteRole < ROLE_Authority) {
		...
		ForEach AllActors(class'PlayerPawn', P)
			if (P != Owner) {
				CR = P.Spawn(Class'RingExplosion2',P,'',Location, Pawn(Owner).ViewRotation);
				CR.bOnlyOwnerSee = True;
			}
	}
}
...
simulated function DoSuperDuperExplosion()
{
	...
	if (RemoteRole < ROLE_Authority) {
		...
		ForEach AllActors(class'PlayerPawn', P)
			if (P != Owner) {
				CR = P.Spawn(Class'RingExplosion3',P,'',Location, Pawn(Owner).ViewRotation);
				CR.bOnlyOwnerSee = True;
			}
	}
}
