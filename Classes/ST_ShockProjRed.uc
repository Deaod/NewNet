class ST_ShockProjRed extends RedShockProj;
...
simulated function Tick(float DeltaTime)
{
	...
	//{
		Texture=Texture'UltimateNewnetv0_6_Database.TeamShock4.R_a00';
	   	LightBrightness=255;
	   	LightHue=21;
	    LightSaturation=64;
	    LightRadius=6;
...
{	
	...
	{
		if (Level.NetMode == NM_Client && !IsA('NN_ShockProjRedOwnerHidden'))
		{
	...
	if(Pawn(Owner) != None)
	{
		Spawn(Class'Red_ComboRing',,'',Location, Pawn(Owner).ViewRotation);
	}
...
{	
    ...
    {
    	RRing = Spawn(Class'Red_SuperComboRing',,'',Location, Pawn(Owner).ViewRotation);
	}
	PlayOwnedSound(ExploSound,,20.0,,2000,0.6);
...
simulated function NN_SuperExplosion(Pawn Pwner)	// aka, combo.
{
	...
	{
		RedRing = Spawn(Class'Red_ComboRing',Pawn(Owner),'',Location, Tater);
	}
	PlaySound(ExploSound,,20.0,,2000,0.6);
    if(bbP != none && Pawn(Owner) != None)
    {
    	bbP.xxClientDemoFix(RedRing, class'Red_ComboRing', Location, RedRing.Velocity, RedRing.Acceleration, Tater);
    }
...
simulated function NN_SuperDuperExplosion(Pawn Pwner)	// aka, combo.
{
	...
	if(bbP != none && Pawn(Owner) != None)
    {
    	RRing = Spawn(Class'Red_SuperComboRing',Pawn(Owner),'',Location, Tater);
	}
	PlaySound(ExploSound,,20.0,,2000,0.6);
    if(bbP != none && Pawn(Owner) != None)
    {
    	bbP.xxClientDemoFix(RRing, Class'Red_SuperComboRing', Location, RRing.Velocity, RRing.Acceleration, Tater);
    }
...
simulated function Explode(vector HitLocation, vector HitNormal)
{
	...
	if (Damage > 60)
	{
		...
		if (bbP != None && Pawn(Owner) != None)
		{
			bbP.xxClientDemoFix(None, class'Red_RingExplosion',HitLocation+HitNormal*8,,, rotator(HitNormal));
		}
	}
	else
	{
...
simulated function NN_Momentum( float DamageRadius, float Momentum, vector HitLocation )
{
	...
	if ( bbP == None || !bbP.bNewNet || Self.IsA('NN_ShockProjRedOwnerHidden') || RemoteRole == ROLE_Authority )
		return;
