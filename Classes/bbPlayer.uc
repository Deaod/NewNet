...
simulated function Touch( actor Other )
{
	if (DisablePortals)
	{
		if (Level.NetMode != NM_Client)
		{
			if (Other.IsA('Kicker'))
				zzbForceUpdate = true;
				zzForceUpdateUntil = ServerTimeStamp + 0.15;
				Disable('Touch');
...
*/
	...
	else
	{
		...
		jumpHeight = zzUTPure.Default.jumpHeight;
		xxSetDoubleJump(bDoubleJump, maxJumps, jumpHeight);
...
simulated function xxSetDoubleJump(bool bDJ, int mJ, float jH)
{
	bDoubleJump = bDJ;
	maxJumps = mJ;
	jumpHeight = jH;
