class ASData extends Mutator;

var Weapon ReplaceThis, ReplaceWith;
var ASSpawnNotify ReplaceSN;
...
function int WeaponReplacement (Actor Other)
{
	...
	{
		return DoReplace(Weapon(Other),class'ST_ShockRifle');
	}

	else if ( ClassIsChildOf( Other.Class, class'PulseGun') )
	{
		if ( Other.Class == class'PulseGun' )
			return DoReplace(Weapon(Other),class'ST_asPulseGun');
		if ( Other.Class == class'ST_PulseGun' )
			return DoReplace(Weapon(Other),class'ST_asPulseGun');
	}
...
event PostBeginPlay()
{
	ReplaceSN = Spawn(class'ASSpawnNotify');
