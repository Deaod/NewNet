class ST_sgShockProjBlue extends ST_sgShockProj;
...
{	
	...
	{
		if (Level.NetMode == NM_Client && !IsA('NN_sgShockProjBlueOwnerHidden'))
		{
	...
	Spawn(Class'Blue_ComboRing',,'',Location, Instigator.ViewRotation);
...
{	
	local bbPlayer bbP;
    local Blue_SuperComboRing Ring;
	...
	Ring = Spawn(Class'Blue_SuperComboRing',,'',Location, Instigator.ViewRotation);
	PlayOwnedSound(ExploSound,,20.0,,2000,0.6);
...
simulated function NN_SuperExplosion(Pawn Pwner)	// aka, combo.
{
	...
	Ring = Spawn(Class'Blue_ComboRing',Pwner,'',Location, Tater);
	PlaySound(ExploSound,,20.0,,2000,0.6);
    if(bbP != none)
    {
        bbP.xxClientDemoFix(Ring, class'Blue_ComboRing', Location, Ring.Velocity, Ring.Acceleration, Tater);
    }
...
simulated function NN_SuperDuperExplosion(Pawn Pwner)	// aka, combo.
{
	...
	Ring = Spawn(Class'Blue_SuperComboRing',Pwner,'',Location, Tater);
	PlaySound(ExploSound,,20.0,,2000,0.6);
    if(bbP != none)
        bbP.xxClientDemoFix(Ring, Class'Blue_SuperComboRing', Location, Ring.Velocity, Ring.Acceleration, Tater);
...
simulated function Explode(vector HitLocation, vector HitNormal)
{
	...
	if (Damage > 60)
	{
		...
		if (bbP != None)
			bbP.xxClientDemoFix(None, class'UT_BlueRingExplosion2',HitLocation+HitNormal*8,,, rotator(HitNormal));
	}
	else
	{
		Spawn(class'UT_BlueRingExplosion2',,, HitLocation+HitNormal*8,rotator(Velocity));
		if (bbP != None)
			bbP.xxClientDemoFix(None, class'UT_BlueRingExplosion2',HitLocation+HitNormal*8,,, rotator(Velocity));
	}
...
simulated function NN_Momentum( float DamageRadius, float Momentum, vector HitLocation )
{
	...
	if ( bbP == None || !bbP.bNewNet || Self.IsA('NN_sgShockProjBlueOwnerHidden') || RemoteRole == ROLE_Authority )
		return;
